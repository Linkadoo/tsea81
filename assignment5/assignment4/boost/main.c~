...
struct lift_msg{
  ...
  int from_floor[MAX_TRAVELS];      // Specify source and destion for the LIFT_TRAVEL message.
  int to_floor[MAX_TRAVELS];
};
...
int find_vacant_spot(lift_type Lift, int floor){
  int i;
...
static void lift_process(void)
{
  ...
  char msgbuf[4096];
  struct lift_msg person_travels[MAX_N_PERSONS];
  while(1){
    int i,ID,count,following_floor,spot;
	...
	if (Lift->passengers_in_lift[i].to_floor == Lift->floor )
	  {
	    if(Lift->travel_count[Lift->passengers_in_lift[i].id] == MAX_TRAVELS)
	      {
		...
		message_send((char *) m, sizeof(*m), QUEUE_FIRSTPERSON +
	    ...
	    else{ //Add the passenger to his next travel
			
	      //Increment travel counter
	      (Lift->travel_count[Lift->passengers_in_lift[i].id])++;
	      ...
	      count = Lift->travel_count[ID];
	      following_floor = person_travels[ID].from_floor[count];
	      spot = find_vacant_spot(Lift,following_floor);
	      Lift->persons_to_enter[following_floor][spot].id = ID;
	      Lift->persons_to_enter[following_floor][spot].to_floor = person_travels[ID].to_floor[count];
	    }
    ...
    case LIFT_TRAVEL:
      ...
      //    Update the Lift structure so that the person with the given ID  is now present on the floor
      person_travels[m->person_id] = *m;
      spot = find_vacant_spot(Lift,m->from_floor[0]);
      Lift->persons_to_enter[m->from_floor[0]][spot].id = m->person_id;
      Lift->persons_to_enter[m->from_floor[0]][spot].to_floor = m->to_floor[0];
      Lift->travel_count[m->person_id] = 0;
      break;
    }
  }
  return;
...
static void person_process(int id)
{
  ...
  while(1){
    if(nr_of_rides >= 500000/MAX_TRAVELS)
      {
    ...
    // Call the lift with the origin and destination floor of the person.
    message_send((char *) m, sizeof(*m), QUEUE_LIFT,0);
    // Wait for the travels to be done.
