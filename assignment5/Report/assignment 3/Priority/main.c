...
int main(int argc, char **argv)
{
  ...
  init_random();
  Lift = lift_create();
  struct sched_param p_vip;
  struct sched_param p_kack;
  ...
  p_vip.sched_priority = 5;
  int i;
  for(i = 0; i < VIP_PERSONS; i++){
    pthread_create(&thread_handle[*current_passenger_id], NULL, passenger_thread,(void*)current_passenger_id);
    if(pthread_setschedparam(thread_handle[*current_passenger_id], SCHED_RR, &p_vip) != 0)
      {
  ...
  p_kack.sched_priority = 1;
  for(i = 0; i<MAX_N_PERSONS-VIP_PERSONS; i++)
    {
      pthread_create(&thread_handle[*current_passenger_id], NULL, passenger_thread,(void*)current_passenger_id);
      if(pthread_setschedparam(thread_handle[*current_passenger_id], SCHED_RR, &p_kack) != 0)
      {
	perror("Could not set the thread priority");
      }
      sem_wait(&init_passenger);
