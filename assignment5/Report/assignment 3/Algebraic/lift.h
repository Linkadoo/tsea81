...
#define MAX_N_PERSONS 20

#define VIP_PERSONS 2

/* maximum number of passengers in lift */ 
#define MAX_N_PASSENGERS 10
/* fig_end lift_h_defs */ 
...
typedef struct
{
  ...
  /* number of leaving passengers on lift */
  int leaving;
 /* number of entering passengers on lift */
  int entering;
 /* number of entering VIP passengers on lift */
  int vip_entering;
  ...
  pthread_cond_t leave_lift;  // Passengers in lift- new floor 
  pthread_cond_t lift_move; // Move the lift
  /* condition variable for floor*/ 
  pthread_cond_t enter_lift[N_FLOORS];  // Persons to enter lift 
  /* condition variable for VIP*/ 
  pthread_cond_t enter_lift_vip;  // VIP to enter lift 
...
/* MONITOR function lift_travel: makes the person with id id perform 
   ...
   at to_floor */ 
void lift_travel(
		 lift_type lift, int id, int from_floor, int to_floor, int vip);
/* fig_end mon_functions */ 

int enter_floor(lift_type lift, int id, int floor, int vip);
